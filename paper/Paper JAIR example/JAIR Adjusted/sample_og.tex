...
\usepackage{jair, theapa, rawfonts, graphicx, pdfpages}

\jairheading{1}{1993}{1-15}{6/91}{9/91}
\ShortHeadings{Fairness and Transparency in ML: A Methodological Framework}
{Salazar, Ruz, \& Tabares-Soto}
\firstpageno{25}
...
\author{\name Nelson Salazar Valdebenito \email nesalazar@alumnos.uai.cl \\
        \addr Faculty of Engineering \& Sciences, Universidad Adolfo Ibáñez,\\
        Santiago 7941169, Chile
        \AND 
       \name Gonzalo A. Ruz \email gonzalo.ruz@uai.cl \\
       \addr Faculty of Engineering \& Sciences, Universidad Adolfo Ibáñez,\\
        Santiago 7941169, Chile\\
        \addr Center of Applied Ecology and Sustainability (CAPES), \\ 
        Santiago, 8331150, Chile\\
        \addr Data Observatory Foundation,\\
        Santiago, 7941169, Chile
       \AND
       \name Reinel Tabares-Soto \email  rtabares@autonoma.edu.co \\
       \addr Department of Electronics and Industrial Automation,\\ Universidad Autónoma de Manizales,\\ Manizales 170001, Colombia\\
       \addr Departamento de Sistemas e Informática, Universidad de Caldas, \\
       Manizales, 170001, Colombia \\
       \addr GobLab School of Government, Universidad Adolfo Ibáñez\\
       Santiago 7941169, Chile\\
       \addr Faculty of Engineering \& Sciences, Universidad Adolfo Ibáñez,\\
        Santiago 7941169, Chile\\}
...
In fact, one way to address these ethical concerns is through regulation. In June 2023, the European Union approved its first AI regulatory act, which sets out harmonised rules for AI systems. The regulation outlines specific bans on certain AI practices, mandates for high-risk systems, and emphasises transparency in AI-human interactions and media manipulation. It also provides a comprehensive framework for market surveillance, covering both providers and users of AI systems in the EU \shortcite{eu-52021PC0206}.  Governments are not the only ones interested in this, industry is too. OpenAI wants regulation for what it calls ``frontier AI'' \shortcite{anderljung2023frontier}, i.e., models whose capabilities could pose serious risks to public safety. 

In addition, AI cannot replicate many human capabilities. In the field of law, a lawyer must combine abstract thinking with skills to solve problems in situations of high uncertainty, in both the legal and factual domains; something a classification model cannot yet achieve \shortcite{surden2014machine}. This is, models cannot ``reason'' in the same manner as a human being. Even the large language models — such as OpenAI's ChatGPT or Google's Bard — that have become so popular recently are not capable of doing so, or at least not independently \shortcite{hao2023reasoning,wei2023chainofthought}. This is due to the fact that behind that black box, there are only probabilities and computations among vectors and matrices. Hence, models should be ``ethically trained'' in order to avoid these problems.
...
These ethical concerns have been previously addressed. Several studies have suggested various guidelines and methodologies for AI ethics \shortcite{Floridi2018,baron2019interpretable,Boddington_2017,Mittelstadt_2019},  but they have not yet converged to a standard in technical and ethical terms \shortcite{Hagendorff2020,fjeld2020principled,Jobin2019,zeng2018linking},  as many of these studies are task-specific and focused on AI development rather than on ethical considerations \shortcite{Ayling2022,Morley2020}.

However, ethical assessment tools can serve several purposes, including identifying disparities and biases in models. These tools, such as What-If-Tool \shortcite{wexler_2018}, SHAP \shortcite{NIPS2017_7062}, AI Fairness 360 \shortcite{aif360-oct-2018}, Fairlearn \shortcite{weerts2023fairlearn}, and Aequitas \shortcite{DBLP:journals/corr/abs-1811-05577}, are effective for analysing model performance, optimising models, and auditing their results. For documentation purposes, there are different tools available such as Model Cards \shortcite{DBLP:journals/corr/abs-1810-03993} and Datasheets for Datasets \shortcite{DBLP:journals/corr/abs-1803-09010}, among others. 
...
\begin{enumerate}
    ...
    \item Evaluate the base model regarding its performance, biases (through counterfactual analysis), and fairness (disparities).
...
We are using two of the datasets that were handed over for this work. These contains the records of multiple individuals accused of drug trafficking or petty theft in between the years 2017 and 2022. The details are as follows:

    \begin{itemize}
        \item There are 16,690 non-null rows for the drug trafficking set; whilst the petty theft has 39,954 non-null rows.
        
        \item Each dataset has a mixture of defendant-related and trial-related columns. The second ones are directly related to the outcome of the trial, so those cannot be used for training purposes. Hence, we are using those related to the defendant: region (geographical location; ``Region''), level of development of the crime (``Desarrollo''), hearings (``Audiencias Efectivas'') and the Barrister (``Defensor''); and exclusively for the drug trafficking set, there is also the foreigner (``Extranjero'') column.  
        ...
        \item Because of this, a new ``Edad'' (age) is created for each set. For the drug trafficking one, we use the information from the Fourteenth National Drug Survey in the General Population of Chile, made by the National Service for the Prevention and Rehabilitation of Drug and Alcohol Consumption \shortcite{EstudioPG2020} in Chile, which states that people between the ages of 18 and 34 years old concentrates the majority of the drug consumption in the country. Hence, for this experiment, we take a normal distribution with a mean of 26 (the average between 18 and 34) and a standard deviation of 8. 
        
        \item For the second dataset, we use the data from the Centre for Crime Studies and Analysis \shortcite{PortalCEAD} for the crime of theft between the years 2017 and 2022. This variable also considers a normal distribution with a mean of 31 (average age between 18 and 44, which accounts for the majority of those charged with this crime) and a standard deviation of 8. 
...
In this section, we look at the essential development and ethical tools used throughout our research. Those in the first category are as follows:

\begin{enumerate}
    \item \textbf{Python}: A versatile and widely-used programming language, Python offers extensive libraries and frameworks for ML and data analysis, facilitating the design and implementation of our algorithms \shortcite{10.5555/1593511}.
    
    \item \textbf{Data processing and visualisation}: This includes libraries such as Pandas \shortcite{reback2020pandas} and Numpy \shortcite{harris2020array} for data manipulation, analysis, and transformation, providing a foundation for our data preprocessing needs. For visualisations, we use Matplotlib \shortcite{Hunter:2007} for basic graphs and plots.
\end{enumerate}

Optimisation tools:

\begin{enumerate}
    \item \textbf{Scikit Learn}: An open-source ML library for Python, Scikit Learn provides simple tools for data analysis and modelling, making it easier to implement ML algorithms \shortcite{scikit-learn}.
...
\begin{enumerate}
    \item The base situation consists of a basic model that can predict the previous mentioned classes. Once the model is created, the results are analysed using SHAP values \shortcite{NIPS2017_7062}, the What If Tool \shortcite{wexler_2018} and Aequitas \shortcite{DBLP:journals/corr/abs-1811-05577}, so that the biases and disparities get identified.
    ...
    \begin{enumerate}
        \item For the pre-training phase, we use AI Fairness 360 \shortcite{aif360-oct-2018} and its Reweighting tool to assign new weights to the sample data, as well as Scikit-Learn's \shortcite{scikit-learn} \textit{compute\_class\_weight} function for optimising the class weights. Some basic hyperparameter tuning was also applied for some gains in model performance.
...
\subsection{Results: Drug trafficking}
...
\begin{itemize}
        ...
        \item \textbf{Datasets}: 70\% split for the training set, 30\% for the testing set, and a 1000 rows were removed from the training dataset to create a validation dataset. In total, there are 10683 rows for training, and 5007 for testing.
...
This new model also uses the improved sample weights that were calculated by AIF360's reweighting function. The class weight were also balanced using Scikit-Learn's \textit{compute\_class\_weight} method, even though it was not really necessary to do so. Model performance for both models are shown in Table \ref{tab_perf_tdrogas:}.
...
SHAP values were calculated by using the data from the training set for both models. Table \ref{tab_shap_abs_tdrogas:} presents the absolute SHAP values, indicating the overall effect on the model, whereas Table \ref{tab_shap_tdrogas:} provides information about its impact on the predictions (if it contributes towards positive or negative predictions).

\begin{table*}[!htb]
\centering
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline
    \textbf{Model} & \textbf{Barrister} &   \textbf{Age} &   \textbf{Region} &  \textbf{Development} &  \textbf{Hearings} &  \textbf{Foreigner} \\ \hline
     Base &   0.006392 & 0.001494 & 0.016019 &     0.000132 &  0.000645 &   0.330375 \\ 
Improved &   0.051984 & 0.004848 & 0.046972 &     0.000172 &  0.002176 &   0.217328 \\ 

\hline
\end{tabular}
\caption{Average absolute SHAP Values by feature.}
\label{tab_shap_abs_tdrogas:}
\end{table*}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=1,width=1\textwidth]{Images/shap_opt_tdrogas.pdf}
    \caption{SHAP values distribution by feature (improved model).}
    \label{fig:shap_opt_tdrogas}
\end{figure}

\subsubsection{Fairness \& Disparities (Aequitas)}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=1,width=1\textwidth]{Images/aeq_test_ref_base_tdrogas.pdf}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=1,width=1\textwidth]{Images/aeq_test_maj_base_tdrogas.pdf}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=1,width=1\textwidth]{Images/aeq_test_min_base_tdrogas.pdf}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=1,width=1\textwidth]{Images/aeq_test_ref_opt_tdrogas.pdf}
    \caption{Model assessment for the reference group.}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=1,width=1\textwidth]{Images/aeq_test_maj_opt_tdrogas.pdf}
    \caption{Model assessment for the major group.}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=1,width=1\textwidth]{Images/aeq_test_min_opt_tdrogas.pdf}
    \caption{Model assessment for the min metrics group.}
...
\subsection{Results: Petty Theft}
...
SHAP Values were calculated using the data from the training set for both models. The averages are laid out in Table \ref{tab_shap_abs_hfalta:} and Table \ref{tab_shap_hfalta:}.

\begin{table}[!htb]
\centering
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline
        \textbf{Model} & \textbf{Barrister} & \textbf{Age} & \textbf{Region} & \textbf{Development} & \textbf{Hearings} \\ \hline
        Base & 0.060050 & 0.004756 & 0.186943 & 0.167577 & 0.377383 \\ 
        Improved & 0.088094 & 0.009539 & 0.183160 & 0.188467 & 0.374409 \\ 
        \hline
    \end{tabular}
\caption{Average absolute SHAP Values by feature. \centering}
\label{tab_shap_abs_hfalta:}
\end{table}
...
\hline
    ...
    \end{tabular}
\caption{Average SHAP Values by feature.}
\label{tab_shap_hfalta:}
\end{table}

The distribution of these values can be found in Figures \ref{fig:shap_base_hfalta} and \ref{fig:shap_opt_hfalta}.

\begin{figure}[!htb]
    \centering
    \includegraphics[scale=1,width=1\textwidth]{Images/shap_base_hfalta.pdf}
    \caption{SHAP values distribution by feature (base model).}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=1,width=1\textwidth]{Images/shap_opt_hfalta.pdf}
    \caption{SHAP values distribution by feature (improved model).}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=1,width=1\textwidth]{Images/aeq_test_ref_base_hfalta.pdf}
    \caption{Model assessment for the reference group.}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=1,width=1\textwidth]{Images/aeq_test_maj_base_hfalta.pdf}
    \caption{Model assessment for the major group.}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=1,width=1\textwidth]{Images/aeq_test_min_base_hfalta.pdf}
    \caption{Model assessment for the min metrics group.}
...
Table \ref{tab_asses_opt_hfalta:} and Figures \ref{fig:aeq_test_ref_opt_hfalta}, \ref{fig:aeq_test_maj_opt_hfalta} and \ref{fig:aeq_test_min_opt_hfalta} are the assessment results for the improved model. Table  \ref{tab_region_hfalta_opt:} is the crosstab for the variable ``Region''.
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=1,width=1\textwidth]{Images/aeq_test_ref_opt_hfalta.pdf}
    \caption{Model assessment for the reference group.}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=1,width=1\textwidth]{Images/aeq_test_maj_opt_hfalta.pdf}
    \caption{Model assessment for the major group.}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=1,width=1\textwidth]{Images/aeq_test_min_opt_hfalta.pdf}
    \caption{Model assessment for the min metrics group.}
...
\subsection{Drug Trafficking}

The performance of the model shows clear differences between the base and improved configurations. As it can be seen in Table \ref{tab_perf_tdrogas:}, the base model excels in accuracy and precision, but the recall is quite low, indicating a tendency towards negative predictions. The improved version, on the other hand, opts for a compromise, favouring a balanced distribution of false negatives and positives, although at the expense of some accuracy and precision.

Examining the base model SHAP values in Table \ref{tab_shap_abs_tdrogas:}, it becomes evident that the variables ``Foreigner'' and ``Region'' predominantly influence the model, with ``Barrister'' and ``Age'' trailing them. Particularly, when looking at Table \ref{tab_shap_tdrogas:}, it can be seen that ``Foreigner'' has on average a negative impact towards predictions, whilst ``Region'' has a mainly positive one, although given its value, its impact is virtually negligible. ``Barrister'' and ``Age'' also have mainly a negligible impact on the predictions. In the improved configuration, the influence of ``Region'' and ``Barrister'' equilibrates, but their influence becomes more pronounced than in the baseline setup, as their SHAP values went up from 0.016019 and 0.006392, to 0.046972 and 0.051984, respectively -- which is a 193.17\% increase for the variable ``Region'', and a 713.23\% for ``Barrister''. On the other hand, ``Foreigner'' reduces its impact by 34.15\%, and now it has a positive influence towards predictions when looking at Table \ref{tab_shap_tdrogas:}, as its average value is now positive. ``Region'' still has a net positive impact in the predictions, and its value is now 22.9 times bigger than in the base model. 

Analysis of Figures \ref{fig:shap_base_tdrogas} and \ref{fig:shap_opt_tdrogas} reveals the dual nature of the ``Foreigner'' variable, which could positively or negatively influence the model. In the base model, lower value regions leans towards negative predictions, a likely reflection of overrepresentation of certain regions, especially from the northern areas. The improved model, however, moderates this, resulting in a more balanced influence of this variable on predictions. The variable ``Barrister'' also shifts in influence, moving from a negligible negative impact in the base model to a still neutral but positive influence post-optimisation, which goes in hand with what was shown in Table \ref{tab_shap_tdrogas:}.
...
Insights from Table \ref{tab_shap_abs_hfalta:} reveals that the base model predominantly relies on the number of hearings to drive decisions. The regional aspects and the development grade of the crime come secondary, whereas factors like age and barrister contributes the less. Table \ref{tab_shap_hfalta:} shows that both ``Hearings'' and ``Barrister'' have a negative influence towards predictions, as its average SHAP Value is negative. The improved model, according to Table \ref{tab_shap_abs_hfalta:}, displays subtle alterations, with the ``Development'' variable slightly edging over ``Region'', as its impact is now 1.12 times bigger than in the base model, while the second variable actually reduced its effect by 2.02\%. However, the amount of hearings is still the dominant variable, having a negligible change compared to the base model (a reduction of 0.79\%). When looking at Table \ref{tab_shap_hfalta:}, ``Region'' now has a more neutral effect, as its SHAP value was reduced by 54.29\% (0.023519 to 0.010753), while, on the other hand, ``Hearings'' is now 1.13 times more influential towards negative predictions. The overall results shown in this table tells us that the optimisations made the variables have a more ``negative'' effect on predictions, as every feature, excepting ``Development'', had a reduction or maintained a negative SHAP value.

Figures \ref{fig:shap_base_hfalta} and \ref{fig:shap_opt_hfalta} basically confirms what was discussed previously, although it can be seen that ``Hearings'' had plenty of cases with big positive effect on the predictions, this makes sense as the minimum SHAP value for this variable was -0.455377 for the base model and -0.464773 for the improved one, while the maximum value was 1.438100 and 1.414085, respectively. It can also be seen that an increased number of hearings tends to enhance predictive results. As for the variable ``Region'', there is no visible tendency or skew towards a kind of prediction, independently of the region.
...
\vskip 0.2in
\bibliography{JAIRAuthorKit/ref}
